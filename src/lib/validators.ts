...
export const applicationSchema = z.object({
  ...
  phone: z.string().min(10, "Telefone inválido").max(50, "Telefone muito longo"),
  linkedin_url: z
    .string()
    .max(500)
    .url("URL inválida")
    .refine((v) => v === "" || v.includes("linkedin.com"), {
      message: "Informe uma URL do LinkedIn",
    })
    .optional()
    .or(z.literal("")),
  portfolio_url: z.string().max(500).url("URL inválida").optional().or(z.literal("")),
  cover_letter: z.string().max(2000, "Máximo de 2000 caracteres").optional().or(z.literal("")),
  salary_expectation: z.string().max(200).optional().or(z.literal("")),
...
export const talentPoolSchema = z.object({
  ...
  brand_interest: z
    .array(z.enum(["welcome_weddings", "welcome_trips", "welcome_group", "corporativo"]))
...
export const jobFormSchema = z.object({
  ...
  contract_type: z.enum(["clt", "pj", "estagio", "temporario"]).optional().or(z.literal("")),
  ...
  closes_at: z
    ...
    .refine(
      ...
      { message: "A data de fechamento deve ser uma data futura" }
    ),
});
